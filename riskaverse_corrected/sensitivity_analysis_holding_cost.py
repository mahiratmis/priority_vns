...
for lamda in [0, 0.5, 1]:
    ...
    for i in range(len(df)):
        ...
        for h_multiplier in [1.25, 1.5, 2.0, 2.5, 3.0]:  # holding cost sensivity
            ...
            holding_costs = h_multiplier * np.array(ast.literal_eval(df['holding_costs'][i]))
            penalty_cost = df['penalty_cost'][i] 
            FailureRates = np.array(ast.literal_eval(df["FailureRates"][i]))
            ServiceRates = np.array(ast.literal_eval(df["ServiceRates"][i]))
            ...
            Parameter_Value.append(h_multiplier)
