...
def optimal_server_number(priority, FailureRates, ServiceRates, holding_costs, penalty_cost, skill_cost, machineCost):
    ...
    holding_backorder_CostList, _, _, _ =simulation_codes.SimulationInterface.simulation_optimization_bathrun_priority_riskaverse(lamda, var_level, FailureRates, ServiceRates,holding_costs, penalty_cost, assignment,priority,
    ...
    while True:
        ...
        temp_holding_backorder_CostList, _, _, _ =simulation_codes.SimulationInterface.simulation_optimization_bathrun_priority_riskaverse(lamda, var_level,FailureRates, ServiceRates,holding_costs, penalty_cost, assignment,priority,
                                  numberWarmingUpRequests = 5000,
                                  stopNrRequests = 100000,
                                  replications =40)
...
def optimal_server_number2(priority, FailureRates, ServiceRates, holding_costs, penalty_cost, skill_cost, machineCost):
    ...
    assignment = np.ones((min_nserver, len(FailureRates)))
    holding_backorder_CostList, holdList, backorderList, SList = simulation_codes.SimulationInterface.simulation_optimization_bathrun_priority_riskaverse(lamda,
                                                                                                                                                   var_level,
                                                                                                                                                   FailureRates,
                                                                                                                                                   ServiceRates,
                                                                                                                                                   holding_costs,
    ...
    while True:
        ...
        # print min_nserver
        assignment = np.ones((min_nserver, len(FailureRates)))
        temp_holding_backorder_CostList, temp_holdList, temp_backorderList, temp_SList = simulation_codes.SimulationInterface.simulation_optimization_bathrun_priority_riskaverse(lamda,
                                                                                                                                                                      var_level,
                                                                                                                                                                      FailureRates,
                                                                                                                                                                      ServiceRates,
                                                                                                                                                                      holding_costs,
...
Results=[]
param_name = "holding_cost"
prefix =  param_name + "_" + prefix
        ...
        if case['simulationGAresults']['skill_cost_factor']==0.1:
            if case['simulationGAresults']['utilization_rate'] ==0.8:
                for hc_multiplier in [1.5, 2]:       
                    ...
                    ### sensitivity pram vals
                    VNS_SimOpt["param_name"]= param_name
                    VNS_SimOpt["param_value"]= hc_multiplier

                    ### metadata calculation
                    min_ser, used_ser, t_cost, ser_cost, h_cost, bo_cost, S = optimal_server_number2(best_priority, FailureRates, ServiceRates, holding_costs, penalty_cost, skillCost, machineCost)
                    VNS_SimOpt["min_server"] = min_ser
                    VNS_SimOpt["used_server"] = used_ser
                    VNS_SimOpt["Server_cost"] = ser_cost
                    VNS_SimOpt["H_cost"] = h_cost
                    VNS_SimOpt["BO_cost"] = bo_cost
                    VNS_SimOpt["T_cost"] = t_cost
                    VNS_SimOpt["Utilization"] = np.sum(FailureRates/ServiceRates) / used_ser
                    VNS_SimOpt["Num_back_orders"] = bo_cost / float(penalty_cost)
                    VNS_SimOpt["S"] = S.tolist()
