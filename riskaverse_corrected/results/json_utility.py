...
def combine_jsons(out_fname="db_results_combined.json",
                  path=".", pattern="*.json"):
    # combine json files in current folder
    with open(out_fname, "w") as outfile:
        outfile.write('[{}]'
                      .format(','.join([open(f, "r").read()
                                       for f in files_with_extension(path,
                                                                     pattern)
                                        ])
                              )
                      )
...
dynamic_part = "_{}_v{}_l{}_".format(db_nodb,
                                     int(var_level*100),
                                     int(lamda*100))
sub_pth = "combined_ga" + dynamic_part + "priority_simopt_riskaverse"
